package com.hopital.accounting.infra.security;
    ...
    private Converter<Jwt, ? extends AbstractAuthenticationToken> jwtAuthenticationConverter() {
        ...
        converter.setJwtGrantedAuthoritiesConverter(jwt -> {
            ...
            if (roles instanceof Collection<?> values) {
                values.stream().filter(Objects::nonNull).map(Object::toString).map(String::trim)
                        .filter(role -> !role.isBlank()).map(role -> new SimpleGrantedAuthority("ROLE_" + role))
